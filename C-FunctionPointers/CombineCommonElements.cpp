...
struct ObjectNode * createIntObjectNode(int *number, int(*equals)(void *, void *)) {
	...
	struct ObjectNode *obj = (struct ObjectNode*)malloc(sizeof(struct ObjectNode));
	obj->value = (void *)number;
	obj->equals = equals;
	return obj;
}
...
struct ObjectNode ** combine_common_elements(struct ObjectNode ** objectNodes1, struct ObjectNode ** objectNodes2, int len1, int len2, int *commonElementsLength) {
	...
	for (i = 0; i < len1; i++)
	{
		...
		if (p == 0)
		{
			for (j = 0; j < len2; j++)
			{
				k = *(int *)objectNodes1[i]->value;
				k1 = *(int *)objectNodes2[j]->value;
				if (k1 == k)
				{
					result[l] = objectNodes1[i]; l++; break;
				}
			}
		}
