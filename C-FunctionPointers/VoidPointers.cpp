...
void typeCastToAllDataTypes(void * long_ptr, void * float_ptr, void * char_ptr, void * str_ptr) {
	...
	//Other code
	printf("\nValue stored at long_ptr is %d", *(int*)long_ptr);
	printf("\nValue stored at long_ptr is %ld", *(int*)float_ptr);
	printf("\nValue stored at long_ptr is %c", *(int*)char_ptr);
	printf("\nValue stored at long_ptr is %s", *(int*)str_ptr);
}
...
void task7() {
	...
	printf("Float is %f", *(float*)getFloatAsVoidStar());
	printf("Int is %d", *(int*)getIntAsVoidStar());
	printf("String is %s", *(char *)getStringAsVoidStar());
}
...
void ** getTwoGreater(void *addr1, void *addr2, void *addr3, int(*compare)(void *, void *)) {
	...
	int i, j, k;
	i=compare(addr1, addr2);
	if (i == 1)
	{
		i = compare(addr1, addr3);
		if (i == 1)
		{
			result[0] = addr1;
			i = compare(addr2, addr3);
			if (i == 1)
			{
				result[1] = addr2;
			}
