...
void pairUp(struct ListNode *head){
	struct ListNode *temp,*temp1,*temp2,*temp3; int i,j;
	if (head != NULL)
	{
		i = 0; temp = head;
		while (temp != NULL)
		...
		j = 0; temp = head; temp1 = head; temp2 = temp;
		while (j < i/2)
		{
			temp = temp1; temp1 = temp1->next; temp2 = temp;
			...
			if (j != i / 2-1&&i%2==0)
			{
				temp->next = temp2; temp2->next = temp1;
				temp3->next = NULL;
			}
			else if (i%2!=0)
			{
