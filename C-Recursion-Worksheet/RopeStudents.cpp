...
void max(struct student *root,int *k1, int k3)
{
	...
	{
		if(root->first!=NULL)max(root->first,k1,k3+1);
		if(root->second!=NULL) max(root->second, k1,k3+1);
		if(root->third!=NULL)max(root->third,k1,k3+1);
	}
}
void min(struct student *root,int *k2, int k3)
{
	if (root->first == NULL&&root->second == NULL&&root->third == NULL)
	{
		if (*k2 == 0 || k3 < *k2){ *k2 = k3; }
	...
	{
		if (root->first != NULL)min(root->first, k2, k3 + 1);
		if (root->second != NULL) min(root->second, k2, k3 + 1);
		if (root->third != NULL)min(root->third, k2, k3 + 1);
	}
}
void findBallDistance(struct student *leaderStudent, int *minBallDistance, int *maxBallDistance)
{
	 max(leaderStudent ,maxBallDistance, 1);
	 min(leaderStudent, minBallDistance, 1);
}
