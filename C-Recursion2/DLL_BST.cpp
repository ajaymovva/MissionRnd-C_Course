...
void fundllbst(struct node_dll **head, struct node_bst *root, int *p);
int is_identical(struct node_dll *head, struct node_bst *root) {
	if (head==NULL||root==NULL)
	return -1;
	else
	{
		struct node_dll *head1; head1 = head;
		struct node_bst *root1; root1 = root;
		int i = 1;
		fundllbst(&head1, root1,&i);
		return i;
	}
}
void fundllbst(struct node_dll **head, struct node_bst *root,int *p)
{
	if (root != NULL&&head!=NULL&&*p==1)
	{
		fundllbst(head, root->left,p);
