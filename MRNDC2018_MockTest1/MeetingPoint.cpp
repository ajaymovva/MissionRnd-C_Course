...
struct CityNode * meetingPoint(struct CityNode * deepikaStartCity, struct CityNode *mayaStartCity, int *numberOfCitiesMayaShouldTravel) {
	...
	{
		common = (struct CityNode*)malloc(sizeof(struct CityNode));
		struct CityNode *temp,*temp1;
		temp1 = deepikaStartCity;
		temp = mayaStartCity;
		k = 0; j = 0;
		while (temp != NULL)
		{
			...
			while (temp1 != NULL)
			{
				if (strcmp(temp->city, temp1->city) == 0)
				{
					k = 1; break;
				}
		...
		{
			common = temp1;
			*numberOfCitiesMayaShouldTravel = j;
			return common;
		}
		else
		{
