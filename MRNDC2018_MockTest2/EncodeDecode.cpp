...
char * encode(char *str) {
	...
	else
	{
		...
		if (l % 2 == 0){ k = l / 2 - 1; j = k + 1; }
		else{ j = l / 2 ; k = j - 1; }i = 0;
		while (j < l&&k >= 0)
		{
			if (l % 2 == 0)
			{
				str1[i++] = str[k]; str1[i++] = str[j]; 
			}
			else
			{
				str1[i++] = str[j]; str1[i++] = str[k];
			}
			k--; j++;
		}
		while (j < l){ str1[i++] = str[j]; j++; }
		while (k >= 0){ str1[i++] = str[k]; k--; }
		str1[i] = '\0';
		return str1;
	}
}
char* decode(char *str) {
	...
	else
	{
		...
		while (j < l&&k >= 0)
		{
			if (l % 2 == 0)
			{
				str1[k] = str[i++]; str1[j] = str[i++];
			}
			else
			{
				str1[j] = str[i++]; str1[k] = str[i++];
			}
			k--; j++;
		}
		while (j < l){ str1[j] = str[i++]; j++; }
		while (k >= 0){ str1[k] = str[i++]; k--; }
		str1[i] = '\0';
		return str1;
	}
}