...
void first(struct node *start, int k, int *shortestarr, int *shortestlen, int psum, int *arr, int *p1no, int p2count, int pmul, int *pmul1);
int mangocity_count_ways(struct node *startcity, int k, int *shortestpath, int *shortestpathlen) {
	...
	if (startcity==NULL||shortestpath==NULL||shortestpathlen==NULL)
	return -1;
	else
	{
		int *arr = (int *)malloc(sizeof(int) * 1000); int p = 0, p1 = 0, p2 = 0,p3=1,p4=1;;;
		first(startcity, k, shortestpath, shortestpathlen, p, arr,&p1,p2,p3,&p4);
		return p1;
...
void first(struct node *start, int k, int *shortestarr, int *shortestlen, int psum, int *arr, int *p1no,int p2count,int pmul,int *pmul1)
{
	if (start != NULL)
	{
		psum = psum + start->data;
		pmul = (pmul)*(start->data);
		arr[p2count] = start->data;
		p2count = (p2count)+1;
	if (k == psum)
	{
		(*p1no)++; 
		if (p2count < *shortestlen||*shortestlen==0)
		{
			*shortestlen = p2count; *pmul1 = pmul;
			for (int i = 0; i < *shortestlen; i++)
			{
				shortestarr[i] = arr[i];
			}
		}
		else if (p2count == *shortestlen)
		{
			if (pmul < *pmul1)
			{
				for (int i = 0; i < *shortestlen; i++)
		...
		first(start->left, k, shortestarr, shortestlen, psum, arr, p1no, p2count, pmul, pmul1);
		first(start->right, k, shortestarr, shortestlen, psum, arr, p1no, p2count,pmul,pmul1);
	}
}